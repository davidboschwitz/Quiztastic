//generates the service for ajax requests
...
//the endpoint for ajax requests
...
.controller('PresenterController', function($scope, io, $interval, $timeout, update, windowDebug) {
    ...
    $interval(function() {
        ...
        //do nothing if time is less than zero or NaN
    ...
    //starts or pairs this presenter with the server
    $scope.pair = function() {
        //calls api
        io.api('start', {
            quizID: $scope.quizID,
    ...
    $scope.next = function() {
        //stops displaying time left
        $scope.data.time = -1;
        //calls api to advance the server
        io.api('next');
        //forces update
        update($scope, true);
    };
